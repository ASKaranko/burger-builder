	...
	state = {
		orderForm: {
			...
			deliveryMethod: {
				...
				value: 'fastest',
				// та же цель, что и ниже с valid
				validation: {},
				// Добавляем valid=true, так как deliveryMethod участвует в цикле
				// по переборе всех элементов формы на валидность
				valid: true
			},
		},
		formIsValid: false,
	...
	orderHandler = (e) => {
		...
		for (let formElementIdentifier in this.state.orderForm) {
			if (this.state.orderForm.hasOwnProperty(formElementIdentifier)) {
				// Получаем только свойство и его value
